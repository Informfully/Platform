...
class InnerArchive extends React.Component {
    ...
    get newsArticles() {
        const { newsArticlesInArchive, experimentConfig } = this.props;
        const { isLoadingInitialSet } = this.state;

        return newsArticlesInArchive.map(({
            _id,
            image,
            body,
            ...article
    ...
    get emptyArchiveMessage() {
        return (
            <Text style={styles().text}>{I18n.t('FAVOURITES_LIST.ARCHIVE_EMPTY')}</Text>
        );
    }

    get hasArticlesInArchive() {
        const { newsArticlesInArchive } = this.props;
        return newsArticlesInArchive.count() > 0;
    }
    ...
    render() {
        ...
        if (!this.hasArticlesInArchive) {
            return (
                <View style={styles().loadingContainer}>
                    {this.emptyArchiveMessage}
                </View>
            );
        }
...
InnerArchive.propTypes = {

    // TODO: Check why we get objects in empty cases (Meteor?)
    newsArticlesInArchive: PropTypes.oneOfType([
        PropTypes.array,
        PropTypes.object,
    ]),
...
export default withTracker(() => {
    ...
    const isLoading = !newsArticlesInArchiveSubscription.ready();
    const newsArticlesInArchive = collectionManager.collection('newsArticlesInArchive').find(
